...
public class AddressBookSimulator {
	...
	public void searchByName() {
		...
		AddressBookSimulator.booksMap.entrySet().forEach(entry -> {
			...
			System.out.println(entry.getValue().getPersonList().stream().filter(
					person -> person.getFirstName().equals(contactName) || person.getLastName().equals(contactName))
	...
	public void showPersonsByCity() {
		...
		for (Map.Entry<String, AddressBook> entry : AddressBookSimulator.booksMap.entrySet()) {
			cityContactList = entry.getValue().getPersonList().stream().filter(person -> person.getCity().equals(city))
	...
	public void sortPersonByZip() {
		AddressBookSimulator.booksMap.entrySet().forEach(entry -> {
			System.out.println("sorted contacts by zip: " + entry.getValue().getPersonList().stream()
					.sorted(Comparator.comparing(Persons::getZip)).collect(Collectors.toList()));
		});
	}
}